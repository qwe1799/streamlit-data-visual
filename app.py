...
with col_right:
    ...
    if page == "航线规划":
        map_type = st.radio("地图", ["高德普通地图", "卫星影像地图"], horizontal=True)
        st.markdown("### ⛰️ 飞行高度")
        ...
        # 接收前端点集并保存
        points = components.html(render_map(latA, lngA, latB, lngB, map_type), height=680)

        # 结束绘制时直接保存，不做任何判断
        if not st.session_state.drawing and isinstance(points, list) and len(points) > 0:
